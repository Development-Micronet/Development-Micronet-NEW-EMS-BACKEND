...
class Department(HorillaModel):
    ...
    def clean(self, *args, **kwargs):
        # Remove ORM-specific parameters before calling parent clean()
...
class RotatingShiftAssign(HorillaModel):
    ...
    def clean(self):
        # Relaxed: allow multiple active rotating shift assigns per employee (for online/offline check-in use case)
        # Previously, this enforced a single active record per employee and raised:
        # "Only one active record allowed per employee"
        # That constraint is now removed intentionally.
        if self.start_date < django.utils.timezone.now().date():
            raise ValidationError(_("Date must be greater than or equal to today"))
...
class BrevoEmailConfiguration(HorillaModel):
    """
    Brevo (Sendinblue) Email Configuration
    For professional, reliable email delivery
    """

    api_key = models.CharField(
        max_length=256,
        verbose_name=_("Brevo API Key"),
        help_text=_("Get your API key from https://app.brevo.com/settings/account/api"),
    )

    from_email = models.EmailField(
        max_length=256,
        default="noreply@horilla.com",
    ...
    from_name = models.CharField(
        ...
        verbose_name=_("From Name (Display Name)"),
    )

    is_active = models.BooleanField(
        default=True,
        verbose_name=_("Active"),
        help_text=_("Activate/deactivate this configuration"),
    )
    ...
    class Meta:
        verbose_name = _("Brevo Email Configuration")
        verbose_name_plural = _("Brevo Email Configurations")

    def __str__(self):
        return f"Brevo Config - {self.from_email}"
    ...
    def test_connection(self):
        ...
        try:
            ...
            accounts_api = sib_api_v3_sdk.AccountApi(
                sib_api_v3_sdk.ApiClient(configuration)
            )
            account = accounts_api.get_account()

            return True, f"✅ Brevo connection successful! Account: {account.email}"
