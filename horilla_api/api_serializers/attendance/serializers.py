...
class AttendanceActivitySerializer(serializers.ModelSerializer):
    ...
    def get_day(self, obj):
        if obj.attendance_date:
            return obj.attendance_date.strftime("%A")
        return None
...
class UserAttendanceDetailedSerializer(serializers.ModelSerializer):
    ...
    leave_breakdown = serializers.SerializerMethodField()
    ...
    def get_is_on_leave(self, obj):
        return LeaveNew.objects.filter(
            employee=obj.employee_id,
            status="approved",
            start_date__lte=obj.attendance_date,
            end_date__gte=obj.attendance_date,
        ).exists()

    def get_leave_breakdown(self, obj):
        leave = (
            ...
            .values_list("start_date_breakdown", flat=True)
            .first()
        )
