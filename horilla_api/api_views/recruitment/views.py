...
from recruitment.test_integration import create_test_candidate
...
class RecruitmentCandidateAPIView(APIView):
    ...
    def post(self, request):
        ...
        # If candidate is created directly in a 'test' stage, trigger ACE integration
        test_integration_result = None
        if (
            candidate.stage_id
            and getattr(candidate.stage_id, "stage_type", None) == "test"
        ):
            technologies = request.data.get("technologies", None)
            test_link = request.data.get("test_link", None)
            try:
                test_integration_result = create_test_candidate(
                    candidate, technologies=technologies, test_link=test_link
                )
                logger.info(
                    "Test integration triggered for new candidate %s (stage=%s): success=%s",
                    candidate.id,
                    candidate.stage_id.stage,
                    test_integration_result.get("success"),
                )
            except Exception:
                logger.exception(
                    "Test integration failed for new candidate %s", candidate.id
                )
        ...
        if test_integration_result:
            response_data["test_integration"] = test_integration_result
    ...
    def put(self, request, pk=None):
        ...
        # Capture the old stage before update to detect stage transitions
        old_stage = candidate.stage_id
        old_stage_type = getattr(old_stage, "stage_type", None) if old_stage else None
        ...
        # Detect stage transition to 'test' — trigger ACE test integration
        test_integration_result = None
        new_stage = updated_candidate.stage_id
        new_stage_type = getattr(new_stage, "stage_type", None) if new_stage else None

        if new_stage_type == "test" and old_stage_type != "test":
            technologies = request.data.get("technologies", None)
            test_link = request.data.get("test_link", None)
            try:
                test_integration_result = create_test_candidate(
                    updated_candidate, technologies=technologies, test_link=test_link
                )
                logger.info(
                    "Test integration triggered for candidate %s (stage: %s → %s): success=%s",
                ...
                logger.exception(
                    "Test integration failed for candidate %s", updated_candidate.id
                )
