...
from horilla_api.api_serializers.base.serializers import DisciplinaryActionSerializer


class DisciplinaryActionView(APIView):
    ...
    def put(self, request, pk):
        ...
        if not action:
            return Response({"error": "Not found"}, status=404)
        serializer = DisciplinaryActionSerializer(action, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=200)
        return Response(serializer.errors, status=400)
...
class WorkTypeRequestView(APIView):
    ...
    def get(self, request, pk=None):
        ...
        from horilla_api.api_serializers.base.work_type_request_comment_serializer import (
        ...
        if pk:
            ...
            comment_serializer = WorkTypeRequestCommentSerializer(comments, many=True)
            data = serializer.data
            data["comments"] = comment_serializer.data
            # Custom response fields
            custom_data = {
                "employee_name": f"{data.get('employee_first_name', '')} {data.get('employee_last_name', '')}",
                "requested_work_type": data.get("work_type_name"),
                "previous_work_type": data.get("previous_work_type_name"),
                "requested_date": data.get("requested_date"),
                "requested_till": data.get("requested_till"),
                "status": (
                    "Approved"
                    if data.get("approved")
                    else ("Canceled" if data.get("canceled") else "Pending")
                ),
        ...
        for item in page:
            ...
            comments = WorkTypeRequestComment.objects.filter(request_id=item)
            comment_serializer = WorkTypeRequestCommentSerializer(comments, many=True)
            results.append(
                {
                    "employee_name": f"{item_data.get('employee_first_name', '')} {item_data.get('employee_last_name', '')}",
                    "requested_work_type": item_data.get("work_type_name"),
                    "previous_work_type": item_data.get("previous_work_type_name"),
                    "requested_date": item_data.get("requested_date"),
                    "requested_till": item_data.get("requested_till"),
                    "status": (
                        "Approved"
                        if item_data.get("approved")
                        else ("Canceled" if item_data.get("canceled") else "Pending")
                    ),
...
class RotatingShiftAssignView(APIView):
    ...
    def get(self, request, pk=None):
        ...
        if pk:
            ...
            if rotating_shift_assign is None:
                return Response({"error": "RotatingShiftAssign not found"}, status=404)
            assignments = [rotating_shift_assign]
        else:
            ...
            assignments = rotating_shift_assigns_filter_queryset

        result = []
        for a in assignments:
            employee_name = str(a.employee_id)
            rotating_shift = a.rotating_shift_id.name if a.rotating_shift_id else None
            based_on = a.based_on
            rotate = a.rotate_after_day
            start_date = a.start_date
            current_shift = a.current_shift.employee_shift if a.current_shift else None
            next_switch = a.next_change_date
            next_shift = a.next_shift.employee_shift if a.next_shift else None
            result.append(
                {
                    "Employee name": employee_name,
                    "Rotating Shift": rotating_shift,
                    "Based on": based_on,
                    "Rotate": rotate,
        ...
        if pk:
            return Response(result[0] if result else {}, status=200)
        return Response({"status": "success", "assignments": result}, status=200)
