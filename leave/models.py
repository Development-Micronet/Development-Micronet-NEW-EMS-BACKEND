...
# Rotating Work Type Assignment Model
class EmployeeRotatingWorkTypeAssignment(models.Model):
    WORK_TYPE_CHOICES = [
        ("WFH to WFO", "WFH to WFO"),
        ("WFO to WFH", "WFO to WFH"),
    ...
    employee = models.ForeignKey(
        Employee, on_delete=models.CASCADE, related_name="rotating_work_assignments"
    )
    rotating_work_type = models.CharField(max_length=32, choices=WORK_TYPE_CHOICES)
    start_date = models.DateField()
    based_on = models.CharField(max_length=16, choices=BASED_ON_CHOICES)
    rotate_after_day = models.PositiveIntegerField(null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    def __str__(self):
        return f"{self.employee} | {self.rotating_work_type} | {self.start_date}"
...
### new leave model starts from here
...
class LeaveNew(models.Model):
    ...
    START_DATE_BREAKDOWN_CHOICES = [
        ("full_day", _("Full Day")),
        ("first_half", _("First Half")),
        ("second_half", _("Second Half")),
    ]

    END_DATE_BREAKDOWN_CHOICES = [
        ("full_day", _("Full Day")),
        ("first_half", _("First Half")),
        ("second_half", _("Second Half")),
    ]

    # 1️⃣ Employee (who is applying)
    employee = models.ForeignKey(
        Employee,
        on_delete=models.CASCADE,
        related_name="leaves",
    ...
    leave_type = models.CharField(
        max_length=20, choices=LEAVE_TYPE_CHOICES, verbose_name=_("Leave Type")
    )
    ...
    start_date_breakdown = models.CharField(
        max_length=20,
        choices=START_DATE_BREAKDOWN_CHOICES,
        default="full_day",
        verbose_name=_("Start Date Breakdown"),
    )
    # 5️⃣ No. of days requested (auto-calculated)
    number_of_days = models.DecimalField(
        max_digits=4, decimal_places=1, editable=False, verbose_name=_("Number of Days")
    )
    ...
    # 9️⃣ Comments (manager / HR comments)
...
class LeaveTypeNew(models.Model):
    PAID_CHOICES = [
        ("paid", "Paid"),
        ("unpaid", "Unpaid"),
    ]
    ...
    is_paid = models.CharField(max_length=10, choices=PAID_CHOICES, default="paid")
