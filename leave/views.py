...
from horilla.decorators import login_required, manager_can_enter, hx_request_required
...
def compensatory_leave_individual_view(request, comp_leave_id):
    """
    View for displaying the details of a single compensatory leave request.
    """
    comp_leave_req = CompensatoryLeaveRequest.objects.filter(id=comp_leave_id).first()
    if not comp_leave_req:
        messages.error(request, _( "Compensatory leave request not found!" ))
        return redirect("view-compensatory-leave")

    context = {
        "comp_leave_req": comp_leave_req,
        "current_date": date.today(),
    }
    return render(request, "leave/compensatory_leave/compensatory_leave_individual_view.html", context)
...
def reject_compensatory_leave(request, comp_id):
    """
    Function to reject a compensatory leave request and reload the list view.
    """
    try:
        ...
        if comp_leave_req.status == "requested":
            ...
            # NOTIFY
            user = getattr(comp_leave_req.employee_id, "employee_user_id", None)
            if user:
...
# API: Employee Rotating Work Type Assignment
from django.views.decorators.csrf import csrf_exempt
from django.views.decorators.http import require_http_methods

from leave.models import EmployeeRotatingWorkTypeAssignment


@csrf_exempt
@require_http_methods(["POST"])
def assign_rotating_work_type(request):
    try:
        data = json.loads(request.body.decode())
        employee_id = data.get("employee_id")
        rotating_work_type = data.get("rotating_work_type")
        start_date = data.get("start_date")
        based_on = data.get("based_on")
        rotate_after_day = data.get("rotate_after_day")
        if not all([employee_id, rotating_work_type, start_date, based_on]):
            return JsonResponse(
                {"status": "error", "message": "Missing required fields."}, status=400
            )
        assignment = EmployeeRotatingWorkTypeAssignment.objects.create(
            employee_id=employee_id,
            rotating_work_type=rotating_work_type,
            start_date=start_date,
            based_on=based_on,
            rotate_after_day=rotate_after_day,
        )
        return JsonResponse(
            {
                "status": "success",
                "message": "Rotating work type assigned.",
...
def get_rotating_work_type(request):
    employee_id = request.GET.get("employee_id")
    if not employee_id:
        return JsonResponse(
            {"status": "error", "message": "employee_id required."}, status=400
        )
    assignments = EmployeeRotatingWorkTypeAssignment.objects.filter(
        employee_id=employee_id
    )
    result = []
    for a in assignments:
        employee_name = (
            a.employee.__str__() if hasattr(a.employee, "__str__") else str(a.employee)
        )
        # Calculate current work type, next switch, next work type
        current_work_type = (
            a.rotating_work_type.split(" to ")[0]
            if " to " in a.rotating_work_type
            else a.rotating_work_type
        )
        next_work_type = (
            a.rotating_work_type.split(" to ")[1]
            if " to " in a.rotating_work_type
            else a.rotating_work_type
        )
        # Next switch date logic
        from datetime import datetime, timedelta

        start_date = a.start_date
        rotate = a.rotate_after_day or 0
        based_on = a.based_on
        if based_on == "After" and rotate:
            next_switch = start_date + timedelta(days=rotate)
        elif based_on == "Weekend":
            next_switch = start_date + timedelta(days=7)
        elif based_on == "Monthly":
            next_switch = start_date + timedelta(days=30)
        else:
            next_switch = None
        result.append(
            {
                "Employee name": employee_name,
                "Rotating Work Type": a.rotating_work_type,
                "Based On": a.based_on,
                "Rotate": rotate,
                "Start Date": str(start_date),
                "Current Work Type": current_work_type,
                "Next Switch": str(next_switch) if next_switch else None,
                "Next Work Type": next_work_type,
            }
        )
...
def update_rotating_work_type(request, assignment_id):
    try:
        assignment = EmployeeRotatingWorkTypeAssignment.objects.get(id=assignment_id)
        data = json.loads(request.body.decode())
        for field in [
            "rotating_work_type",
            "start_date",
    ...
    except EmployeeRotatingWorkTypeAssignment.DoesNotExist:
        return JsonResponse(
            {"status": "error", "message": "Assignment not found."}, status=404
        )
    except Exception as e:
        return JsonResponse({"status": "error", "message": str(e)}, status=400)
...
def delete_rotating_work_type(request, assignment_id):
    try:
        assignment = EmployeeRotatingWorkTypeAssignment.objects.get(id=assignment_id)
        assignment.delete()
        return JsonResponse({"status": "success", "message": "Assignment deleted."})
    except EmployeeRotatingWorkTypeAssignment.DoesNotExist:
        return JsonResponse(
            {"status": "error", "message": "Assignment not found."}, status=404
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if save:
                ...
                if multiple_approvals_check(leave_request.id):
                    ...
                    with contextlib.suppress(Exception):
                        send_employee_notification(
                            ...
                            description="You have a new leave request to validate.",
                ...
                send_admin_notification(
                    request.user,
                    verb="Leave application submitted",
                    description=(
                        f"{leave_request.employee_id} submitted a leave application "
        ...
        if form.is_valid():
            ...
            if save:
                ...
                send_employee_notification(
                    ...
                    description=f"Leave request updated for {leave_request.employee_id}.",
                    target=leave_request,
                    level="info",
    ...
    if leave_request.status != "approved":
        if total_available_leave >= leave_request.requested_days:
            ...
            if send_notification:
                # NOTIFY
                user = getattr(leave_request.employee_id, "employee_user_id", None)
                if user:
                    send_employee_notification(
                        request.user,
                        user,
            ...
            if leave_request.status != "rejected":
                ...
                if user:
                    send_employee_notification(
                        ...
                        description=(
                            f"Your leave request from {leave_request.start_date} to "
                            f"{leave_request.end_date} has been rejected."
                        ),
                        target=leave_request,
                        level="error",
                        icon="close-circle-outline",
            ...
            if request.method == "POST":
                ...
                if form.is_valid():
                    ...
                    send_admin_notification(
                        ...
                        description=(
                            f"{leave_request.employee_id} cancelled a leave request "
                            f"from {leave_request.start_date} to {leave_request.end_date}."
                        ),
                        target=leave_request,
                        level="info",
...
def delete_allocation_comment_file(request):
    ...
    if (
        ...
        messages.warning(request, _("You don't have permission"))
        script = f"""
            <span hx-get='/leave/allocation-request-view-comment/{leave_id}/?&amp;target=leaveRequest" hx-target="#commentContainer" hx-trigger="load"></span>
        """
    return HttpResponse(script)
    ...
    def approve_compensatory_leave(request, comp_id):
        ...
        try:
            ...
            if comp_leave_req.status == "requested":
                ...
                send_employee_notification(
                    request.user,
                    comp_leave_req.employee_id,
                    verb="Comp-off added",
                    description="Compensatory leave has been added to your account.",
                    target=comp_leave_req,
                    level="success",
...
#################################################################
##                                                              #
##         view code for leave starts here bro                  #
##                                                              #
#################################################################


### ADMIN API VIEW FOR LEAVE MANAGEMENT
...
class AdminLeaveAPIView(APIView):
...
## Employee API VIEW FOR LEAVE MANAGEMENT
from rest_framework.views import APIView

from .leaveserializer import (
    AdminAssignLeaveSerializer,
    LeaveNewCreateSerializer,
    LeaveNewEmployeeReadSerializer,
...
class EmployeeLeaveAPIView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    # 🔹 UPDATE LEAVE (ONLY IF REQUESTED)
    def put(self, request, pk):
        try:
            leave = LeaveNew.objects.get(pk=pk, employee=request.user.employee_get)
        except LeaveNew.DoesNotExist:
        ...
        serializer = LeaveNewCreateSerializer(
            leave,
            data=request.data,
            partial=True,
        )
    ...
    # 🔹 DELETE LEAVE (ONLY IF REQUESTED)
    def delete(self, request, pk):
        ...
        if leave.status != "requested":
            return Response(
                {"error": "Only requested leaves can be deleted"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class AdminLeaveTypeAPIView(APIView):
    permission_classes = [IsAuthenticated]

    @method_decorator(permission_required("leave.add_leavetype", raise_exception=True))
    def post(self, request):
        # 🔒 role check
        if request.user.employee_get.role != "admin":
            return Response(
    ...
    def get(self, request):
        leave_types = LeaveTypeNew.objects.all().order_by("name")
        serializer = LeaveTypeCreateSerializer(leave_types, many=True)
        return Response(
            {
    ...
    @method_decorator(
        permission_required("leave.change_leavetypenew", raise_exception=True)
    )
    def put(self, request, pk):
        if request.user.employee_get.role != "admin":
            return Response(
        ...
        try:
            leave_type = LeaveTypeNew.objects.get(pk=pk)
        except LeaveTypeNew.DoesNotExist:
            return Response(
                {"error": "Leave type not found"},
                status=status.HTTP_404_NOT_FOUND,
            )

        serializer = LeaveTypeCreateSerializer(
            leave_type, data=request.data, partial=True
        )
        serializer.is_valid(raise_exception=True)
        serializer.save()
    ...
    # 🔹 DELETE LEAVE TYPE
    @method_decorator(
        permission_required("leave.delete_leavetypenew", raise_exception=True)
    )
    def delete(self, request, pk):
        if request.user.employee_get.role != "admin":
            return Response(
...
class AdminAssignLeaveAPIView(APIView):
    permission_classes = [IsAuthenticated]

    @method_decorator(permission_required("leave.add_leavenew", raise_exception=True))
    def post(self, request):
        # 🔒 Role check
        if request.user.employee_get.role != "admin":
            return Response(
        ...
        leave_type = LeaveTypeNew.objects.get(
            id=serializer.validated_data["leave_type_id"]
        )

        today = timezone.localdate()
        end_date = today + timedelta(days=leave_type.total_days - 1)
        ...
        leave = LeaveNew.objects.create(
            ...
            start_date=today,
            end_date=end_date,
            status="approved",  # 👈 admin assigned
...
class LeaveBalanceSummaryAPIView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request):

        employees = Employee._base_manager.all().order_by("employee_first_name")
        leave_types = LeaveTypeNew.objects.all()
