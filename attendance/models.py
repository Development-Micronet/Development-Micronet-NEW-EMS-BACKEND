...
class AttendanceActivity(HorillaModel):
    ...
    is_late_come = models.BooleanField(default=False, verbose_name=_("Is Late Come"))
    is_early_out = models.BooleanField(default=False, verbose_name=_("Is Early Out"))
    ...
    def save(self, *args, **kwargs):
        ...
        LATE_THRESHOLD = dt_time(10, 30)
        EARLY_THRESHOLD = dt_time(18, 30)

        # Helper logic to ensure times are actual time objects
        check_in = self.clock_in
        ...
        check_out = self.clock_out
        if isinstance(check_out, datetime):
            check_out = check_out.time()
        elif isinstance(check_out, str):
            # Try parsing H:M or H:M:S depending on what your API sends
            try:
        ...
        if check_in:
            self.clock_in = check_in
        if check_out:
            self.clock_out = check_out

        # Ensure we have a date and employee before running DB queries
        if self.attendance_date and self.employee_id:
            # 1. Logic for Late Come
            if check_in:
                previous_activities = AttendanceActivity.objects.filter(
                    employee_id=self.employee_id,
                    attendance_date=self.attendance_date,
                    clock_in__lt=check_in,
                ).exclude(pk=self.pk)

                self.is_late_come = (
                    not previous_activities.exists() and check_in > LATE_THRESHOLD
                )

            # 2. Logic for Early Out
            if check_out:
                future_activities = AttendanceActivity.objects.filter(
                    employee_id=self.employee_id,
                    attendance_date=self.attendance_date,
                    clock_in__gt=check_out,
                ).exclude(pk=self.pk)

                self.is_early_out = (
                    not future_activities.exists() and check_out < EARLY_THRESHOLD
                )
            else:
...
class Attendance(HorillaModel):
    ...
    def save(self, *args, **kwargs):
        # Recalculate worked hour before saving
        self.attendance_worked_hour = format_time(self.get_at_work_from_activities())
