...
class OnboardingStage(HorillaModel):
    ...
    def clean(self):
        super().clean()
        if self.is_final_stage and self.recruitment_id:
            queryset = OnboardingStage.objects.filter(
            ...
            if queryset.exists():
                raise ValidationError(
                    {
                        "is_final_stage": _(
                            "Only one final stage is allowed for a recruitment."
                        )
                    }
                )

    def save(self, *args, **kwargs):
        if self.sequence is None and self.recruitment_id:
            max_sequence = (
                OnboardingStage.objects.filter(recruitment_id=self.recruitment_id)
                .exclude(pk=self.pk)
                .aggregate(max_sequence=Max("sequence"))
                .get("max_sequence")
            )
            self.sequence = 0 if max_sequence is None else max_sequence + 1
        self.full_clean()
