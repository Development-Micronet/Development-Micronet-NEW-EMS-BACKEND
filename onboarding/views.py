...
from base.methods import closest_numbers, generate_pdf, get_pagination, sortby
...
from onboarding.serializers import CandidateSerializer
...
VALID_TASK_STATUSES = {choice[0] for choice in CandidateTask.choice}
...
def _sync_stage_tasks_for_candidate(candidate, stage):
    """
    Keep candidate tasks in sync with the candidate's current onboarding stage.
    """
    stage_tasks = OnboardingTask.objects.filter(stage_id=stage)
    stage_task_ids = list(stage_tasks.values_list("id", flat=True))

    # Remove tasks from other stages in the same recruitment.
    CandidateTask.objects.filter(
        candidate_id=candidate,
        stage_id__recruitment_id=stage.recruitment_id,
    ).exclude(onboarding_task_id__in=stage_task_ids).delete()

    # Ensure all stage tasks are assigned to candidate and candidate task rows exist.
    for onboarding_task in stage_tasks:
        onboarding_task.candidates.add(candidate)
        CandidateTask.objects.get_or_create(
            candidate_id=candidate,
            onboarding_task_id=onboarding_task,
            defaults={"stage_id": stage, "status": "todo"},
        )

    CandidateTask.objects.filter(
        candidate_id=candidate, onboarding_task_id__in=stage_task_ids
    ).exclude(stage_id=stage).update(stage_id=stage)
...
def stage_creation(request, obj_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return stage_save(stage, request, obj_id)
    return render(request, "onboarding/stage_form.html", {"form": form, "id": obj_id})


def stage_save(stage, request, rec_id):
    """
    ...
    response = render(
        request,
        "onboarding/stage_form.html",
        {"form": OnboardingViewStageForm(), "id": rec_id},
    )
    return HttpResponse(
...
def task_update(
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            selected_candidate_ids = set(task.candidates.values_list("id", flat=True))
            existing_candidate_tasks = onboarding_task.candidatetask_set.all()
            existing_candidate_ids = set(
                existing_candidate_tasks.values_list("candidate_id", flat=True)
            )

            for cand_task in existing_candidate_tasks:
                if cand_task.candidate_id_id not in selected_candidate_ids:
                    cand_task.delete()
                    continue
                if cand_task.stage_id_id != task.stage_id_id:
                    cand_task.stage_id = task.stage_id
                    cand_task.save(update_fields=["stage_id"])

            for candidate in task.candidates.exclude(id__in=existing_candidate_ids):
...
def candidates_view(request):
    """
    View hired candidates and support optional JSON response.
    """
    ...
    candidates = page_obj
    if field:
        candidates = group_by_queryset(
            candidate_filter_obj.qs, field, request.GET.get("page"), "page"
        ).object_list
    return render(
        request,
...
def candidate_filter(request):
    """
    Filter hired candidates. Supports HTML and optional JSON response.
    """
    queryset = Candidate.objects.filter(
        is_active=True,
        hired=True,
    ...
    if _is_json_request(request):
        if field:
            grouped = general_group_by(candidates, field, page_number, "page")
            data = [
                {
                    "group": group[0],
                    "candidates": CandidateSerializer(group[1], many=True).data,
                }
                for group in grouped
            ]
            return Response({"grouped_results": data})
    ...
    if field:
        candidates = group_by_queryset(
            candidates, field, page_number, "page"
        ).object_list
        return render(
            request,
            "onboarding/group_by.html",
            {
                "candidates": candidates,
    ...
    return render(
        ...
        {
            "candidates": paginator_qry(candidates, page_number),
            "pd": request.GET.urlencode(),
        },
    )
...
def candidate_task_update(request, taskId):
    ...
    if status not in VALID_TASK_STATUSES:
        return JsonResponse(
            {"message": _("Invalid task status"), "type": "danger"}, status=400
        )
    ...
    if request.POST.get("single_view"):
        ...
        if candidate_task is None:
            return JsonResponse(
                {
                    "message": _("Task is not assigned to this candidate"),
                    "type": "danger",
                },
                status=404,
            )
...
def candidate_stage_update(request, candidate_id, recruitment_id):
    ...
    with transaction.atomic():
        ...
        _sync_stage_tasks_for_candidate(candidate, stage)
    onboarding_stages = OnboardingStage.objects.all()
    choices = CandidateTask.choice
    users = [
        employee.employee_user_id
...
def candidate_stage_bulk_update(request):
    ...
    stage_obj = OnboardingStage.objects.get(id=stage)
    updated_ids = []
    with transaction.atomic():
        for cand_stage in CandidateStage.objects.select_related("candidate_id").filter(
            candidate_id__id__in=candidate_id_list
        ):
            if cand_stage.onboarding_stage_id_id != stage_obj.id:
                cand_stage.onboarding_stage_id = stage_obj
                cand_stage.save()
                _sync_stage_tasks_for_candidate(cand_stage.candidate_id, stage_obj)
            updated_ids.append(cand_stage.candidate_id_id)
    type = "info"
