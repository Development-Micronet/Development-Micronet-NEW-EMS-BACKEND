...
class Employee(models.Model):
    ...
    role = models.CharField(
        ...
        default="user",
        help_text=_("Application role of the employee"),
    )
    choice_gender = [
        ("male", trans("Male")),
        ("female", trans("Female")),
    ...
    is_tokenized = models.BooleanField(
        default=False, help_text="Grants tokenized API access visibility when True"
    )
    ...
    def check_online(self):
        ...
        Allows multiple clock-in/clock-out cycles per day by only checking today's attendance.
        """
        if apps.is_installed("attendance"):
...
class EmployeeWorkInformation(models.Model):
    # Direct input fields for names and details (not FKs)
    Department_Name = models.CharField(max_length=100, null=True, blank=True)
    Job_Position_Name = models.CharField(max_length=100, null=True, blank=True)
    Job_Role_Name = models.CharField(max_length=100, null=True, blank=True)
    ...
    Work_Type_Name = models.CharField(max_length=100, null=True, blank=True)
    Employee_Type_Name = models.CharField(max_length=100, null=True, blank=True)
    Employee_Tag_Label = models.CharField(max_length=100, null=True, blank=True)
    Work_Location_Label = models.CharField(max_length=100, null=True, blank=True)
    Company_Name = models.CharField(max_length=100, null=True, blank=True)
    Work_Email_Label = models.CharField(max_length=254, null=True, blank=True)
    Work_Phone_Label = models.CharField(max_length=50, null=True, blank=True)
    Joining_Date_Label = models.DateField(null=True, blank=True)
    Contract_End_Date_Label = models.DateField(null=True, blank=True)
    Basic_Salary_Label = models.DecimalField(
        max_digits=12, decimal_places=2, null=True, blank=True
    )
    Salary_Per_Hour_Label = models.DecimalField(
        max_digits=12, decimal_places=2, null=True, blank=True
    )
    """
...
class WorkTypeRequest(models.Model):
    ...
    employee = models.ForeignKey(
        Employee,
        on_delete=models.CASCADE,
        related_name="work_type_requests",
        verbose_name=_("Employee"),
    )
    ...
    permanent_request = models.BooleanField(default=False)

    def __str__(self):
        return f"{self.employee.get_full_name()} - {self.work_type} ({self.status})"
...
class Policy(HorillaModel):
    ...
    def delete(self, *args, **kwargs):
        # Collect attachment ids before deleting this policy, since M2M
        # cannot be accessed reliably after the instance is deleted.
        attachment_ids = list(self.attachments.values_list("id", flat=True))
        super().delete(*args, **kwargs)
        if attachment_ids:
            PolicyMultipleFile.objects.filter(id__in=attachment_ids).delete()
