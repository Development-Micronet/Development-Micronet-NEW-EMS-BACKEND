...
class ProjectNew(models.Model):
    ...
    managers = models.ManyToManyField(
        Employee, related_name="managed_projects_new", blank=True
    )

    members = models.ManyToManyField(
        Employee, related_name="member_projects_new", blank=True
    )
    ...
    def project_file(self):
        # Backward compatibility for serializers/templates that still access
        # obj.project_file after DB column removal.
        return None


### new task model starts here
...
from .models import ProjectNew  # import your ProjectNew model
...
class TaskNew(models.Model):
    ...
    # 🔹 Project reference (project details come from ProjectNew)
    project = models.ForeignKey(
        ProjectNew,
        on_delete=models.CASCADE,
        related_name="tasks",
    ...
    # 🔹 Stage reference - ForeignKey to ProjectNewStage
    project_stage = models.CharField(
        max_length=100, null=True, blank=True, verbose_name=_("Project Stage")
    )

    # 🔹 Task managers (multiple)
    task_managers = models.ManyToManyField(
        Employee,
        related_name="managed_tasks_new",
        blank=True,
        verbose_name=_("Task Managers"),
    )

    # 🔹 Task members (multiple)
    task_members = models.ManyToManyField(
        Employee,
        related_name="member_tasks_new",
        blank=True,
        verbose_name=_("Task Members"),
    )

    status = models.CharField(
        max_length=20, choices=TASK_STATUS, default="to_do", verbose_name=_("Status")
    )
...
from .models import TaskNew


class TimeSheetNew(models.Model):
    ...
    hours_spent = models.DecimalField(
        ...
        decimal_places=2,
        verbose_name=_("Hours Spent"),
        help_text=_("Hours spent on task (e.g. 7.50)"),
    )
    ...
    def __str__(self):
        return f"{self.employee} | {self.task.title} | {self.date}"


class ProjectNewStage(models.Model):
    """
    Stage/Workflow model for ProjectNew
    - Tracks different stages of project workflow
    - Connected to ProjectNew for workflow management
    ...
    title = models.CharField(max_length=200, verbose_name=_("Stage Title"))

    sequence = models.IntegerField(default=0, verbose_name=_("Sequence Order"))
    ...
    class Meta:
        unique_together = ["project", "title"]
        ordering = ["sequence"]
        verbose_name = _("Project New Stage")
        verbose_name_plural = _("Project New Stages")
