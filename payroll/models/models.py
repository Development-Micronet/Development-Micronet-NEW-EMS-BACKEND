...
class EmployeeContract(models.Model):
    ...
    def clean(self):
        if (
            self.status == "active"
            and EmployeeContract.objects.filter(employee=self.employee, status="active")
            .exclude(id=self.id)
            .exists()
        ):
            raise ValidationError("Active contract already exists for this employee")

    def __str__(self):
        return f"{self.employee} - {self.contract_type} ({self.status})"


##################################
#
#  Pay new model here boy yeah
#
##################################
...
class PayslipNew(HorillaModel):
    ...
    employee = models.ForeignKey(
        Employee, on_delete=models.PROTECT, related_name="payslips_new"
    )
